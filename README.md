...
**M6 — 最近文件、轴向覆盖、贴图路径、截图与关联启动（已完成第 1~4、6 项）**
...
> 第 1~4、6 项已完成（见上），编号沿用 `docs/设计文档.md` §22 的顺序。

| 顺序 | 内容 | 依赖 |
|---|---|---|
| 5 | HDR / EXR 环境贴图导入 | 需要第二条文件选取链路 + 主题引用策略（已定：把 HDR 复制进应用数据目录） |
| 7 | 打包与性能回归 | `pnpm tauri build` + 大模型基准 |
