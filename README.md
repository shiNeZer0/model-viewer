...
- **FBX 兼容性提示**：加载中的降级情况（不支持的贴图通道、多层贴图只留第一层、骨骼共用、Z-up 自动转换）
  会翻译成中文提示列进信息面板；文件声明的单位（`UnitScaleFactor`）会**自动预选「模型单位」**；
  PBR 材质不受 three 支持时回退为黏土材质并说明；旧版本 FBX（7.0 以下）给出可操作的报错（见设计文档 §35）
...
- **FBX 的格式边界**（都来自 three 的加载器，不是本项目实现缺口，界面上会给中文说明）：FBX 6.x 及更早（`FileVersion < 7000`）**无法打开**；PBR / Stingray 材质不受支持（会回退为黏土材质）；多层贴图只保留第一层；部分贴图通道与图片格式会被跳过；Z-up 只认 `UpAxis === 2`（`UpAxisSign` 为负或 X-up 的文件需要手动切「模型轴向」）。
