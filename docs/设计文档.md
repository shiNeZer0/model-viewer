...
`viewer_settings` 建议键：`display.shadingMode`、`display.background`、`display.showGrid`、`display.showAxes`、`display.unit`、`display.upAxis`、`lighting.activePresetId`、`lighting.toneMapping`、`viewer.autoRotate`、`viewer.autoRotateSpeed`、`perf.maxPixelRatio`、`perf.warnFileSizeMB`、`perf.wireframeTriangleLimit`、`perf.idlePauseSeconds`、`security.grantMode`。
...
`computeViewDistance(radius, fovDeg, aspect)` 在 `fovDeg` 缺省/为 NaN 时会算出 **NaN 相机坐标**（`undefined * Math.PI`）。M0 的调用点总会传入 `camera.fov` 所以没暴露，但把算法抽成可复用模块后就成了真实隐患。现已在函数内做合法区间校验（0 < fov < 180，否则回退 50°），并补了 `非法 fov 回退` 的回归用例。

### 16.3 验证结果

| 项 | 结果 |
|---|---|
| `pnpm test` | **107/107 通过**（12 个文件；M1 新增 viewPresets / shadeModes / postfx / idlePolicy / useHotkeys 五组） |
| `pnpm build` | 成功（后处理与 addons 随 Viewer 懒加载分块） |
| `cargo test` | 24/24 通过（M1 未改动 Rust） |
| GUI 验收 | 待用户在本机 `pnpm tauri dev` 中确认：8 种着色模式切换与还原、6 视图方向正确、渐变/透明背景、饱和度与曝光即时生效、静止 5 秒后 CPU 归零 |
